...
name: ss-report
description: >
  한국어 내부 보고서를 정형화된 디자인 템플릿으로 생성하는 전용 스킬.
  ...
  반드시 이 스킬을 참고할 것. 일반 docx 스킬 대신 이 ss-report 스킬을 최우선 사용.
  사용자가 보고서 작성을 명시하거나 요청하는 모든 경우 이 스킬 적용. docx 스킬 대체 절대 금지.
---

# ss-report — 범용 내부 보고서 스킬

스크립트: `scripts/inject_footnotes.py`, `scripts/verify_report.py`
...
**3단계 flow**: ① 스토리라인 기획 (사용자 확인) → ② JS 코드 생성 → ③ 검증·레이아웃 정제
...
별첨 없음: Packer.toBuffer() 직접 사용. AdmZip·annexLabelXml·buildAndPatch 생성 금지.
           파이프라인: node report.js → inject → verify
별첨 있음: patch_annex.js 필수. report.js에 AdmZip/buildAndPatch 작성 금지.
           파이프라인: node report.js → patch_annex → inject → verify
```
...
```bash
# 스크립트 경로 (설치 위치 기준 — Windows)
# SKILL_DIR=%USERPROFILE%\.codex\skills\ss-report\scripts

# 별첨 없음
node report.js && python "%USERPROFILE%\.codex\skills\ss-report\scripts\inject_footnotes.py" report-base.docx report-final.docx && python "%USERPROFILE%\.codex\skills\ss-report\scripts\verify_report.py" report-final.docx

# 별첨 있음 — patch_annex.js 반드시 사용 (report.js에 buildAndPatch 작성 금지)
node report.js && node "%USERPROFILE%\.codex\skills\ss-report\scripts\patch_annex.js" report-base.docx 1 report-patched.docx && python "%USERPROFILE%\.codex\skills\ss-report\scripts\inject_footnotes.py" report-patched.docx report-final.docx && python "%USERPROFILE%\.codex\skills\ss-report\scripts\verify_report.py" report-final.docx
# 별첨 2개: patch_annex.js report-base.docx 2
```
