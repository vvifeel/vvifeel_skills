...
```
[삽입 기준] 독자가 본문을 읽다가 참조하고 싶을 법한 상세 데이터를 표로 정리할 수 있을 때
  - 본문에서 언급한 수치·비교·현황의 상세 내역을 표로 구조화 가능한 경우
  - 없어도 본문 이해는 되지만, 있으면 독자가 찾아볼 만한 참조 데이터

[삽입 금지] 아래에 해당하면 별첨 생성 금지 — 본문에 포함하거나 생략
  - 줄 글 형태의 부연 설명 (→ 본문에 통합)
  - 본문 내용과 중복
  - 없어도 보고서 이해에 지장 없는 것

[형태] 표 우선. 텍스트 별첨은 표로 전환 불가능한 경우에만 허용

[크기·분리]
  - 별첨 1개 = 최대 1페이지 (초과 금지)
  - 반 페이지 미만도 허용 — 표 1~3개도 무방
  - 1페이지 초과 시: 표 축소 또는 별도 주제로 별첨2·별첨3 분리
```
...
- 별첨 1개 = 최대 1페이지 (초과 금지). 내용이 넘치면 표 축소 또는 별첨 분리
- 내용: 본문에 담지 못한 상세 표·참조 데이터 위주
