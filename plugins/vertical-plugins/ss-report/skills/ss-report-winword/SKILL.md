...
풋노트 `*` 위치: **반드시 `fnPrefix()` 사용** — 본문 텍스트를 상수로 분리하고 `_isFn: \`${fnPrefix(본문상수, "용어")}* 설명\`` 형식으로 작성. `scripts/fn_align.js`의 `fnPrefix()`가 용어 시작 위치 아래에 `*` 근사 정렬. "기호 시작점+2칸"·anchor 방식 사용 금지.
...
□ 풋노트 * 위치: fnPrefix(본문상수, "용어")로 정렬 — scripts/fn_align.js require / "기호 시작점+2칸"·anchor 방식 금지
