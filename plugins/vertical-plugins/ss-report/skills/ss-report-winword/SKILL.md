...
description: >
  ss-report-full 기반 + Windows MS Word COM 레이아웃 검증 루프가 추가된 최고 품질 보고서 스킬.
  ...
  반드시 이 스킬을 참고할 것. Windows + MS Word 환경에서 ss-report-full 대신 이 스킬 우선 적용.
  비Windows 환경에서는 ss-report-full 사용.
---

# ss-report-winword — Word COM 검증 포함 최고 품질 보고서 스킬

스크립트: `scripts/inject_footnotes.py`, `scripts/verify_report.py`, `scripts/verify_word_layout.py`, `scripts/patch_annex.js`
...
**3단계 flow + Word COM 검증 루프**: ① 스토리라인 기획 (사용자 확인) → ② JS 코드 생성 → ③ 구조 검증 + Word COM 레이아웃 루프

**빠른 실행 예외**: 사용자 요청에 "빠른 실행", "바로 작성", "초안", "테스트", "스킵" 등 확인 생략 의도가 명확하면 목차·페이지 확인을 내부 판단으로 대체하고 바로 생성한다.

> **⚠️ 사전 조건**: Windows + MS Word 설치 + `pip install pywin32`
> 미충족 시 verify_word_layout.py 실행 불가 — ss-report-full 사용 권장
...
쿼리와 데이터를 분석해 최적 목차를 구성한 뒤 제안. A형/B형 고정 패턴 없음.

**진행 조건: 사용자 확인 후 진행 (목차·페이지 구성은 보고서 품질의 핵심)**
- 목차 제안 → 사용자 확인 → 페이지 계획표 제안 → 사용자 확인 → 코드 생성 순서 준수
- 단, 사용자 요청에 빠른 실행 관련 문구가 있으면 목차 확인을 생략하고 내부 확정 후 바로 진행

**제안 형식:**
```
[목차 제안]
구성 이유: (이 흐름이 적합한 이유 1~2줄)
...
→ 이 구성으로 진행할까요? 수정 원하시면 말씀해 주세요.
```

**원칙:** lv1 기준 3~5개 섹션 / 이유를 함께 제시하여 사용자가 판단 근거를 알 수 있도록
...
```
별첨 없음: Packer.toBuffer() 직접 사용. AdmZip·annexLabelXml·buildAndPatch 생성 금지.
           파이프라인: node report.js → inject → verify → verify_word_layout
별첨 있음: patch_annex.js 필수. report.js에 AdmZip/buildAndPatch 작성 금지.
           파이프라인: node report.js → patch_annex → inject → verify → verify_word_layout
```

### 페이지 계획표 (코드 작성 전 작성 — 사용자 확인 후 JS 시작)

**⚠️ 목차 승인 직후 페이지 계획표를 사용자에게 제시하고, 확인 전 JS 코드 생성 시작 금지**
단, 사용자 요청에 빠른 실행 관련 문구가 있으면 사용자에게 보여주지 않고 내부 계획만 작성한 뒤 JS 코드 생성으로 진행한다.

| 페이지 | 주요 내용 | 예상 단락 수 | 표 포함 여부 | 3줄 케이스 | 사전 조치 |
|--------|---------|------------|------------|-----------|---------|
| P1 | H1+H2+body | 약 N | 없음 | 없음 | — |
| P2 | H1+표(N행) | 약 N + 표 | 있음(N행≈X DXA) | A: 표 잔량 3행↓ 가능성 | 직전 분량 N줄 조정 |
...
풋노트 `*` 위치: **반드시 `_isFn: { anchor:"단어", text:"설명" }` 사용** — 풋노트가 필요한 단어 시작 바로 아래에 `*`가 위치하도록 anchor로 지정. "기호 시작점+2칸" 기본값 사용 금지. `anchor`는 위치 계산용이며 최종 출력은 `* 설명`만 표시.
풋노트 길이: **1줄 절대 초과 금지** (2줄 금지). 초과 위험 시 → 자간 tcs(s, -8) → tcs(s, -12) → 설명 텍스트 단축 순서로 반드시 1줄 안에 완결.
...
셀 내 허용 글자 수 초과 시 처리 순서 (STRICT):
  1. 자간 조절: tcs(s, -8) → tcs(s, -12) (최대 0.6pt) — 1~2자 초과 시
  2. 내용 축약: 문맥 손실 없는 선에서 1~2자 압축
  3. 개행(CLines): 위 두 가지로 해결 불가하고 내용 축약 시 문맥 손실이 클 때만
     → 반드시 단어 경계에서 분리 (음절 중간 절대 금지)
     → CLines(['앞 단어 묶음', '뒤 단어 묶음'], width)
     → 최대 2줄 (3줄 이상 개행 금지)

셀 정렬 원칙:
  C() 가운데 정렬이 기본.
  설명/비고/개요/주요 내용/근거/리스크 등 문장형 장문 컬럼에만 CL()/CLines() 사용.
```
...
## FLOW 3. 검증 + Word COM 레이아웃 루프

### Step A — 구조 검증 (inject + verify_report)

```bash
# 별첨 없음
node report.js && python "scripts/inject_footnotes.py" report-base.docx report-final.docx && python "scripts/verify_report.py" report-final.docx

# 별첨 있음
node report.js && node "scripts/patch_annex.js" report-base.docx 1 report-patched.docx && python "scripts/inject_footnotes.py" report-patched.docx report-final.docx && python "scripts/verify_report.py" report-final.docx
# 별첨 2개: node scripts/patch_annex.js report-base.docx 2
```

한 줄로 연결 실행 → 첫 실패 시 즉시 중단, 원인 파악 후 코드 수정.
실패 0개 확인 후 Step B 진행.

⚠️ 별첨 있을 때 report.js에 AdmZip / buildAndPatch 코드 작성 절대 금지
   → patch_annex.js 가 모든 XML 패치를 담당

### Step B — Word COM 레이아웃 검증 (verify_word_layout)

```bash
python "scripts/verify_word_layout.py" report-final.docx
```

**출력 형식 예시:**
```
=== Word 레이아웃 검증: report-final.docx ===
총 페이지: 2
...
[고립 탐지]
  ⚠ P1 말미 단독 항목: '□ 스크립트, 출력 파일'

[이슈 요약] 1건 발견 → 수정 후 재생성 필요
```

### Step C — 이슈 분석 + report.js 수정

verify_word_layout.py 출력을 읽고 아래 기준으로 defs 배열 수정:

| 이슈 | 탐지 조건 | 조치 (defs 수정) |
|------|---------|----------------|
| **H2 섹션 페이지 걸침** | H2 + 하위 lv3/lv4가 복수 페이지에 걸침 | ① 해당 H2 내 lv3 항목 1~2개 삭제 또는 lv4 제거로 압축<br>② 그래도 안 되면 H2 구조 재편: lv3 항목을 이전/다음 H2로 이동하거나 두 H2를 합침 |
| **페이지 H3/H4 시작** | P(n)이 lv3/lv4로 시작 (H2 없이) | 해당 H2 내용 압축 (위와 동일) |
| **H1/H2 페이지 말미 고립** | H1/H2 직후 다른 페이지로 넘어감 | 직전 body 1~2줄 압축 또는 삭제 |
| **표 페이지 분리** | first_page ≠ last_page | 표 직전 body 1~2줄 삭제하여 표 전체를 다음 페이지로 |
| **섹션이 예상 외 페이지** | 계획과 다른 페이지 배치 | 해당 섹션 body 분량 조정 |
| **총 페이지 초과** | 계획 페이지 수 초과 | 전체 body 1줄씩 균등 압축 |

**H2 섹션 걸침 수정 우선순위:**
```
1순위: 해당 H2의 lv4(·) 항목 제거 → 가장 비용 적은 압축
2순위: lv3(-) 항목 1개 삭제 (중요도 낮은 항목부터)
3순위: lv3 항목 내 문장 단축 (자간 조정 포함)
4순위: H2 구조 재편 — 항목을 인접 H2로 이동하거나 H2 통합
→ 내용 품질을 해치는 수준이면 사용자에게 보고 후 판단 요청
```
...
### Step D — 재생성 + 재검증 루프 (최대 3회)

```
report.js 수정 → node report.js → inject_footnotes.py → verify_report.py → verify_word_layout.py
→ [이슈 요약] ✅ 없음 확인 시 완료 → 출력
```

3회 루프 후에도 미해결 이슈가 남으면: 현재 상태 출력 + 이슈 내용 사용자에게 보고.
...
□ 별첨 필요 여부 판단 (표 반 페이지 초과 또는 참고자료 성격)
...
□ 연도 apostrophe: ' 직접 입력 — inject가 &apos;→' 자동 복원
□ 풋노트: 위치가 아니라 필요성 기준으로만 삽입 / 필요 없으면 본문·표 모두 생략
□ 풋노트 * 위치: 반드시 anchor 사용 — 해당 단어 시작 바로 아래 배치 / "기호 시작점+2칸" 기본값 사용 금지
□ 풋노트 길이: 1줄 절대 초과 금지 — 초과 시 자간(-8→-12) → 설명 단축 순서로 1줄 안에 완결
...
□ 별첨: Document.sections 분리 / annexMarker(n) + annexTitlePara() + 내용
   ...
   텍스트 기반 annexDefs: LV.H2부터 시작 (LV.H1 금지) / 별첨 1개 = 반드시 1페이지
...
□ &apos; 정규화: buildAndPatch() + inject_footnotes.py 자동 처리
□ 머릿글·바닥글 없음 / datePara() 날짜만
□ 날짜·연도·분기·나열 표현 통일
```

**verify_report.py → verify_word_layout.py 순서**: 구조 실패 0개 → Word COM 이슈 0건 확인 → 출력
