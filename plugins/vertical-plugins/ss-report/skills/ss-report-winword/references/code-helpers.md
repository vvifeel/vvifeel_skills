...
// fn: 풋노트 — 파랑 인라인 단락 직접 생성 (inject 불필요)
// _isFn은 문자열만 허용 (anchor 방식 폐기, 고정 indent 사용)
...
//   금지: 위치 균형 맞추기 위한 억지 추가 / SUB+SUB2 항목 / lv1(H1) 전체
//   별첨 언급: 본문 이해에 직접 필요한 경우에만 / 풋노트로 "* 별첨N" 처리
const defs = [
  { lv: LV.H1,   text: "1. 개요" },
  { lv: LV.H2,   text: "□ 추진 배경" },
...
// ── SECTION 6: Document 생성 + fixApos + main() ──────────────────────────
// fixApos: docx 라이브러리가 ' → &apos; escape하는 것을 복원 (inject 불필요)
// 별첨 없음 버전 — 별첨 있으면 references/code-helpers-annex.md 참조
async function main() {
  const doc = new Document({
    sections: [{ properties: { page: { size:{width:11906,height:16838}, margin:{top:1304,bottom:1304,left:1134,right:1134,gutter:0} } }, children }]
  });
  ...
  zip.writeZip("report-final.docx");
  console.log("✅ report-final.docx 생성 완료");
}
main();
